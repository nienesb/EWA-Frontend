...
import { AppPage } from './announcement.po';
import { browser, element, by } from 'protractor';

describe('AMBU Flow Evaluation', function() {
  let page: AppPage;
  ...
  it('Should display the Announcement title', () => {
    browser.ignoreSynchronization = false;
    let parent = element(by.className('nav-title'));
    let child = parent.element(by.tagName('span'));
    expect(child.getText()).toEqual('Aankondiging');
  });
